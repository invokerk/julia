...
This choice of behavior is slightly controversial, but your humble documentation
author (Jeff in this case) likes it.
...
(in `src/subtype.c`) for each variable in the environment.
A variable is diagonal when `!occurs_inv && occurs_cov > 1`.
...
Since the variable must be concrete, a contradiction occurs if its lower bound is
*not* concrete: while a concrete type can be a *subtype* of many other types, it is not
possible for a concrete type to be a *supertype* of an abstract type, so the algorithm
stops with the answer `false` in that case.
